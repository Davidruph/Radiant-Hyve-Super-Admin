            ...
            <div className='grid grid-cols-12 2xl:gap-x-6 lg:gap-x-5 gap-4'>
                {dashboardDetails && dashboardDetails.map((item, index) => (
                    <div className='xl:col-span-3 lg:col-span-4 sm:col-span-4 col-span-12' key={index}>
                        <div className='bg-[#FFF0E6] py-[25px] hover:shadow-lg duration-300 cursor-pointer px-[24px] rounded-lg flex items-center 2xl:gap-x-6 lg:gap-x-4 md:gap-4 gap-3 ' onClick={() => navigate(item.path)}>
                            <div className='bg-[#FFFFFF] rounded-full 2xl:w-[70px] 2xl:h-[70px] xl:w-[65px] xl:h-[65px] lg:w-[58px] lg:h-[58px] md:w-[50px] md:h-[50px] w-[48px] h-[48px] flex items-center justify-center'>
                                <img src={item?.image} alt='...' className='2xl:w-[40px] xl:w-[35px] lg:w-[30px] w-7' />
                            </div>
                            <div>
                                <p className='text-[#4B5563] text-lg font-semibold'>{item?.name}</p>
                                <p className='text-[#4B5563] text-lg font-medium'>{item?.count}</p>
                            </div>
                        </div>
                    </div>
                ))}
                <div className='bg-[#FFFFFF] py-4 md:px-4 px-3 rounded-lg col-span-12 mt-5'>
                    <h2 className='text-[#1F1F1F] font-medium text-lg mb-4'>Subscription School List</h2>
                    <div className="overflow-x-auto">
                        <table className="min-w-full text-sm">
                            <thead className="bg-[#F8FAFB]">
                                <tr>
                                    {["schoolId", "schoolName", "schoolAddress", "plan", "startDate", "endDate"].map((col) => (
                                        <th
                                            key={col}
                                    ...
                                    <th className="p-5 text-left text-[#3B4045] font-medium md:text-base text-sm whitespace-nowrap">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                {paginatedData.map((school, index) => (
                                    <tr key={index} className="hover:bg-gray-50">
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.schoolId}</td>
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.schoolName}</td>
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.schoolAddress}</td>
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.plan}</td>
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.startDate}</td>
                                        <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-2 md:text-base text-sm font-normal whitespace-nowrap">{school.endDate}</td>
                    ...
                    <Dialog.Description className="">
                        <div className='md:px-8 px-3 py-5'>
                            <div className='w-full relative sm:text-center px-2 text-start mb-10'>
                                <h1 className='md:text-2xl text-lg font-semibold text-[#1F1F1F]'>Subscription School Details</h1>
                            ...
                            <div className='mt-7'>
                                <div className='px-2 space-y-5'>
                                    <div className='flex items-center justify-between'>
                                        <h4 className='text-[#4B5563] font-normal md:text-base text-sm'>School Name</h4>
                                        <span className='text-[#1F1F1F] font-normal md:text-base text-sm'>Marvin Cooper</span>
                                    </div>
                                    <div className='flex items-center justify-between'>
                                        <h4 className='text-[#4B5563] font-normal md:text-base text-sm'>School Address</h4>
                                        <span className='text-[#1F1F1F] font-normal md:text-base max-w-72 text-end text-sm'>3891 Preston Rd., South Dakota, California 62639</span>
                                    </div>
                                    <div className='flex items-center justify-between'>
                                        <h4 className='text-[#4B5563] font-normal md:text-base text-sm'>Subscription Plan</h4>
                                        <span className='text-[#1F1F1F] font-normal md:text-base text-sm'>Monthly</span>
                                    </div>
                                    <div className='flex items-center justify-between'>
                                        <h4 className='text-[#4B5563] font-normal md:text-base text-sm'>School Id</h4>
