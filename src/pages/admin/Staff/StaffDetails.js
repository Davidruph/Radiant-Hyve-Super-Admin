...
export default function StaffDetails() {
  ...
  const handleEditData = () => {
    if (staffData) {
      const newSelected =
        genderOptions.find(
          (option) => option.value === (staffData.gender || "")
        ) || genderOptions[0];
      setSelectedGender(newSelected);
      setImage(staffData.profile_pic || "");
      setDob(
        staffData?.dob ? moment(staffData.dob, "YYYY-MM-DD").toDate() : null
      );
      setJoinDate(
        staffData?.joining_date
          ? moment(staffData.joining_date, "YYYY-MM-DD").toDate()
          : null
      );
    }
  ...
  const initialValues = {
    ...
    email: staffData?.email || "",
    phone: staffData?.mobile_no
      ? `+${staffData.country_code}${staffData.mobile_no}`
      : "",
    gender: staffData?.gender || "",
    dob: staffData?.dob || null,
    joinDate: staffData?.joining_date || null,
    experience: staffData?.experience || "",
    aboutStaff: staffData?.about_staff || ""
  };
  ...
  const handleSubmit = (values) => {
    ...
    const formattedJoin = moment(values.joinDate, "YYYY-MM-DD").format(
      "YYYY-MM-DD"
    );
    ...
    formData.append("about_staff", values?.aboutStaff);
  ...
  const handleDelete = () => {
    ...
    setBtnLoader(true);
    deleteStaffApi(id, deleteReason)
      .then((res) => {
        if (res.status === 200) {
  ...
  const handleBlock = () => {
    if (!staffData?.is_blocked && !blockReason.trim()) {
      setReasonError("Reason is required to block the staff.");
      return;
    }
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-12 gap-5">
        <div className="lg:col-span-4 col-span-12 bg-white h-full rounded-lg p-5">
          <div className="flex flex-col items-center justify-center h-full">
            {loader ? (
              <Skeleton circle width={138} height={138} />
            ) : (
              <img
                src={staffData?.profile_pic || PlaceholderImg}
                className="object-cover w-[138px] rounded-full h-[138px]"
                alt=""
              />
            )}

            <h3 className="text-[#0F1113] font-medium w-64 truncate text-center md:text-base text-sm mt-4">
              {" "}
              {loader ? (
            ...
            <h3 className="text-[#4B5563] font-normal w-64 truncate md:text-sm text-center text-xs mt-3">
              Staff Id :{" "}
              <span className="text-[#1F1F1F] font-normal md:text-sm text-xs">
                {" "}
                {loader ? <Skeleton width={50} /> : staffData?.id}
              </span>
            </h3>
            <h3 className="text-[#4B5563] font-normal w-64 truncate md:text-sm text-center text-xs mt-3">
              Experience :
              <span className="text-[#1F1F1F] font-normal md:text-sm text-xs">
                {" "}
                {loader ? <Skeleton width={50} /> : staffData?.experience} Year
              </span>
            </h3>
          </div>
        </div>
        ...
        <div className="lg:col-span-8 col-span-12 bg-white rounded-lg p-5">
          ...
          <div className="mt-7 space-y-5">
            {[
              { label: "Full Name", value: staffData?.full_name },
              {
                label: "Phone Number",
                value: `${staffData?.country_code} ${staffData?.mobile_no}`
              },
              { label: "Gender", value: staffData?.gender },
              { label: "Date of Birth", value: staffData?.dob },
              { label: "Joining Date", value: staffData?.joining_date }
            ].map((item, idx) => (
              <div key={idx} className="flex items-center justify-between">
                <h4 className="text-[#4B5563] text-sm font-normal whitespace-nowrap">
                  {item.label}
                </h4>
            ...
            <div>
              ...
              <p className="text-[#1F1F1F] mt-1.5 font-normal text-sm">
                {" "}
                {loader ? (
                  <Skeleton height={20} count={3} className="w-full" />
                ) : (
                  staffData?.about_staff
                )}
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-12 gap-5 mt-5">
        <div className="lg:col-span-4 col-span-12 bg-white rounded-lg p-5">
          <div className="flex sm:flex-row flex-col items-center gap-3 border-b border-[#F3F4F6] pb-3 justify-between">
            <h2 className="text-[#1F1F1F] font-semibold md:text-lg text-base whitespace-nowrap">
              Assigned Student
            </h2>
            <div>
              <button
                className="bg-[#FFF7E7] px-3 py-2 rounded-xl"
                onClick={() =>
                  navigate(`/school_admin/staff/staff_details/${id}/students`)
                }
              >
                <span className="text-[#4B5563] font-normal md:text-base text-sm whitespace-nowrap">
                  View details
                </span>
              </button>
            </div>
      ...
      <div className="grid grid-cols-12 gap-5">
                  ...
                  <table className="min-w-full text-sm table-fixed">
                    <thead className="bg-[#F8FAFB] block sticky top-0 z-10">
                      <tr className="table w-full table-fixed">
                        <th className="p-4 text-[#3B4045] font-medium text-sm  text-center m-auto first:rounded-l-md last:rounded-r-md whitespace-nowrap">
                          Date
                        </th>
                        <th className="p-4 text-[#3B4045] font-medium text-sm  text-center m-auto first:rounded-l-md last:rounded-r-md whitespace-nowrap">
                          Clock In
                    ...
                    <tbody>
                      {timeEntries.map((Item, index) => (
                        ...
                        >
                          <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-4 text-center text-sm font-normal whitespace-nowrap">
                            {Item?.date}
                          </td>
                          <td className="border-b border-[#E5E7EB] text-[#1F1F1F] px-4 py-4 text-center m-auto text-sm font-normal whitespace-nowrap">
                            {Item?.clock_in_time
        ...
        <div className="lg:col-span-7 col-span-12 bg-[#FFFFFF] py-4 md:px-4 px-3 rounded-lg">
          ...
          <Dialog.Description className="">
            <div className="py-4">
              <div className="w-full relative sm:text-center text-start my-3 md:px-8 px-4">
                <h1 className="md:text-xl text-base font-semibold text-[#274372]">
                  Staff Information
                </h1>
                <button
              ...
              >
                {({ values, setFieldValue, isSubmitting }) => (
                  <Form className="mt-7 w-full">
                    <div className="md:px-10 px-4 w-full h-[550px] modalheight scroll overflow-y-auto">
                        ...
                        <div className="w-full">
                          ...
                          >
                            <div className="relative text-sm">
                              <Listbox.Button className="relative w-full border border-[#D1D5DB] rounded-lg bg-white py-3 pl-3 pr-10 text-left cursor-pointer focus:outline-none">
                                <span
                                ...
                                <Listbox.Options className="absolute z-10 mt-1 w-full bg-white rounded-lg shadow-lg max-h-60 py-1 text-base ring-1 ring-black ring-opacity-5 focus:outline-none">
                                  {genderOptions.map((item, idx) => (
                                    <Listbox.Option
                                      key={idx}
                                      className={({ active }) =>
                                        `relative cursor-pointer select-none py-2 px-5 border-b last:border-none border-[#E9E9E9] ${
                                          active ? "bg-gray-100" : ""
                                        }`
                                      }
                                      value={item}
                      ...
                      <div className="mt-4 w-full text-start">
                        ...
                        <Field
                          as="textarea"
                          name="aboutStaff"
                          placeholder="Write something about staff"
                          className="h-[100px] w-full border border-[#E5E7EB] text-sm px-4 py-3 focus:ring-2 focus:ring-gray-400 focus:outline-none rounded-lg"
                        />
                        <ErrorMessage
                          name="aboutStaff"
                          component="div"
                    ...
                    <Dialog.Description className="">
                        <div className='md:px-8 px-3 py-5'>
                            <div className='w-full relative sm:text-center text-start md:my-3 my-0'>
                                <h1 className='md:text-lg text-base mr-2 font-semibold text-[#1F1F1F]'>Delete Teacher Profiles</h1>
                                <button className='absolute top-0 right-0' onClick={() => setDeleteModal(false)}>
                                    <IoMdClose className='text-2xl text-black' />
                                </button>
                            </div>
                            <div className='mt-7'>
                                <div className='px-2'>
                                    <div className='w-full flex items-center justify-center'>
                                        <img src={DeleteModalIcon} className='w-[104px] h-[104px]' alt='' />
                                    </div>
                                    <div className='w-full mt-5 text-center'>
                                        <h4 className='text-[#1F1F1F] font-semibold md:text-lg text-base mb-4'>Are you sure you want to delete an account?</h4>
                                        <p className='text-[#4B5563] font-normal md:text-base text-sm'>This decision is final and cannot be undone.
                                            Please confirm to continue.</p>
                                    </div>
                                </div>
                                <div className='mt-10 flex justify-between w-full mb-3'>
                                    <button type='button' onClick={() => setDeleteModal(false)} className='bg-[#DFE3EA] w-full h-12 font-medium text-sm text-[#6B7280] rounded-lg mr-5'>Cancel</button>
                                    <button disabled={btnLoader} type='submit' className='bg-[#FF7373] text-white font-medium text-sm w-full h-12 rounded-lg'
            ...
            >
                <Dialog.Panel className="rounded-2xl">
                    <Dialog.Description className="">
                        <div className='md:px-8 px-3 py-5'>
                            <div className='w-full relative sm:text-center text-start md:my-3 my-0'>
                                <h1 className='md:text-lg text-base mr-2 font-semibold text-[#1F1F1F]'>Restrict Access for Staff</h1>
                                <button className='absolute top-0 right-0' onClick={() => setBlockModal(false)}>
                                    <IoMdClose className='text-2xl text-black' />
                                </button>
                            </div>
                            <div className='mt-7'>
                                <div className='px-2'>
                                    <div className='w-full flex items-center justify-center'>
                                        <img src={blockModalIcon} className='w-[104px] h-[104px]' alt='' />
                                    </div>
                                    <div className='w-full mt-5 text-center'>
                                        <h4 className='text-[#1F1F1F] font-semibold md:text-lg text-base mb-4'>Are you sure you want to {staffData?.is_blocked ? "unblock " : "Block"}?</h4>
                                        <p className='text-[#4B5563] font-normal md:text-base text-sm'>This action will restrict the staff access. You can unblock it later if needed. Please confirm to proceed.</p>
                                    </div>
                                </div>
                                <div className='mt-10 flex justify-between w-full mb-3'>
                                    <button type='button' onClick={() => setBlockModal(false)} className='bg-[#DFE3EA] w-full h-12 font-medium text-sm text-[#6B7280] rounded-lg mr-5'>Cancel</button>
                                    <button disabled={btnLoader} type='submit' className='bg-[#FF7373] text-white font-medium text-sm w-full h-12 rounded-lg'
                                        onClick={handleBlock}>
                                        {btnLoader ?
                                            <DotLoader color="#fff" />
                                            : `Yes, ${staffData?.is_blocked ? "Unblock " : "Block"} `
                                        }
                                    </button>
                                </div>
                            </div>
        ...
        <Dialog.Panel className="rounded-2xl">
          <Dialog.Description>
            <div className="md:px-8 px-3 py-5">
              <div className="w-full relative sm:text-center text-start md:my-3 my-0">
                <h1 className="md:text-lg text-base mr-2 font-semibold text-[#1F1F1F]">
                  {staffData?.is_blocked
                    ? "Restore Access for Staff"
              ...
              <div className="mt-7 px-2">
                ...
                <div className="w-full mt-5 text-center">
                  <h4 className="text-[#1F1F1F] font-semibold md:text-lg text-base mb-4">
                    ...
                    {staffData?.is_blocked ? "Unblock" : "Block"} this Staff?
                  </h4>
                  <p className="text-[#4B5563] font-normal md:text-base text-sm">
                    {staffData?.is_blocked
                      ? "This will restore the staff’s access. You can block again later if needed."
                      : "This action will restrict the staff’s access. You can unblock it later if needed. Please confirm to proceed."}
                  </p>
