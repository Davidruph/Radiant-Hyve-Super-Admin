...
import { changePasswordSchoolApi, deleteSchoolApi, editSchoolApi, getSchoolApi } from '../../services/api_services';
            ...
            {loader ? <OvalLoader /> :
                <div className='grid grid-cols-12 gap-5 w-full'>
                    <div className='xl:col-span-4 col-span-12 bg-white rounded-xl p-4'>
                        <h2 className='text-[#1F1F1F] font-semibold md:text-lg text-base mb-5'>Staff Details</h2>
                        <div className='w-full'>
                            {StaffData.map((item, index) => (
                                <div key={index} className='bg-[#F9F9F9] p-5 my-4 rounded-xl w-full flex items-center justify-between'>
                                    <h3 className='text-[#3B4045] font-medium text-base'>{item.title}</h3>
                                    <span className='text-[#3B4045] md:text-xl text-lg font-semibold'>{item.value}</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className='xl:col-span-8 col-span-12 space-y-5'>
                        {schoolData.map((section, sectionIndex) => (
                            <div key={sectionIndex} className='bg-white p-5 rounded-xl w-full'>
                                <div className='w-full flex sm:flex-row flex-col sm:items-center items-start sm:space-y-0 space-y-2 justify-between'>
                                    <h3 className='text-[#1F1F1F] font-semibold text-base'>{section.section}</h3>
                                    {section.section === "School Details" ? (
                                        <div className='flex items-center gap-2'>
                                            <button className='bg-[#FFF7E7] px-3 py-2 flex items-center gap-2 rounded-xl' onClick={() => setEditModal(true)}>
                                                <img src={EditIcon} className='w-[20px] h-[20px]' alt='Edit' />
                                                <span className='text-[#4B5563] font-normal text-sm'>Edit School</span>
                                            </button>
                                            <button className='bg-[#FFDED8] px-3 py-2 flex items-center gap-2 rounded-xl' onClick={() => setDeleteModal(true)}>
                                                <img src={DeleteIcon} className='w-[20px] h-[20px]' alt='Delete' />
                                                <span className='text-[#4B5563] font-normal text-sm'>Delete</span>
                                            </button>
                                        </div>
                                    ) : (
                                        <button className='bg-[#F9F9F9] px-3 py-2 flex items-center gap-2 rounded-xl' onClick={() => setEditAccountModal(true)}>
                                            <img src={EditIcon} className='w-[20px] h-[20px]' alt='Edit' />
                                            <span className='text-[#4B5563] font-normal text-sm'>Edit Details</span>
                                        </button>
                                    )}
                                </div>
                                ...
                                {section.details.map((detail, detailIndex) => (
                                    <div key={detailIndex} className='w-full flex sm:flex-row flex-col sm:items-center items-start justify-between mb-5'>
                                        <h3 className='text-[#4B5563] font-normal text-sm'>{detail.label}</h3>
                                        <p className='text-[#1F1F1F] font-normal text-sm'>
                                            {detail.label === "Password" ? '*'.repeat(detail.value.length) : detail.value}
                                        </p>
                                    </div>
                                ))}
                            </div>
                    ...
                    <Dialog.Description className="">
                        <div className='md:px-8 md:py-4 p-4'>
                            <div className='w-full relative sm:text-center text-start my-3'>
                                <h1 className='md:text-xl text-lg font-semibold text-[#274372]'>Update School Information</h1>
                                <button className='absolute top-0 right-0' onClick={() => setEditModal(false)}>
                                                ...
                                                <div className="mb-4 text-start">
                                                    ...
                                                    <GooglePlacesAutocomplete
                                                        ...
                                                        selectProps={{
                                                            ...
                                                            isClearable: true,
                                                            styles: customGoogleplaceStyles(formik.touched.schoolAddress && formik.errors.schoolAddress),
                                                            name: "schoolAddress",
                                                    ...
                                                    />
                                                    <ErrorMessage name="schoolAddress" component="div" className="text-red-500 text-sm mt-1" />
                                                </div>
                                            </div>
                                        </div>
                ...
                <Dialog.Panel className="rounded-2xl">
                    <Dialog.Description className="">
                        <div className='md:px-8 px-3 py-5'>
                            <div className='w-full relative sm:text-center text-start my-3'>
                                <h1 className='md:text-xl text-lg font-semibold text-[#274372]'>Update Account Information</h1>
                                <button className='absolute top-0 right-0' onClick={() => setEditAccountModal(false)}>
                                    <IoMdClose className='text-2xl text-black' />
                                </button>
                            </div>
                            <Formik
                    ...
                    <Dialog.Description className="">
                        <div className='md:px-8 px-3 py-5'>
                            <div className='w-full relative sm:text-center text-start md:my-3 my-0'>
                                <h1 className='md:text-xl text-lg mr-2 font-semibold text-[#274372]'>Delete School Account Permanently</h1>
                            ...
                            <div className='mt-7'>
                                <div className='w-full flex items-center justify-center'>
                                    <img src={DeleteModalIcon} className='w-[80px] h-[80px]' alt='' />
                                </div>
                                <div className='w-full mt-5 text-center'>
                                    <h4 className='text-[#1F1F1F] font-semibold text-lg mb-4'>Are you sure you want to delete an account?</h4>
                                    <p className='text-[#4B5563] font-normal text-base'>This decision is final and cannot be undone. <br />
                                    ...
                                    />
                                    {deleteReasonError && (
                                        <p className="text-red-500 text-start text-xs">{deleteReasonError}</p>
                                    )}
                                </div>
                            </div>
                            <div className='mt-5 flex justify-between w-full mb-3'>
                                ...
                                <button disabled={btnLoader} type='button' className='bg-[#FF7373] text-white font-medium text-sm w-full h-12 rounded-lg'
                                    onClick={() => handleDelete()}
                                >
                                    {btnLoader ? <DotLoader color="#fff" /> : 'Yes, Delete Account'}
                                </button>
                            </div>
                        </div>
                    </Dialog.Description>
